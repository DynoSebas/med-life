...
export default function EditPatientPage({ params }: EditPatientPageProps) {
  ...
  useEffect(() => {
    const fetchPatient = async () => {
      try {
        ...
        // Mapear datos del paciente al formato del formulario
        setFormData({
          patient: {
            ...
            address_json: {
              ...
              ciudad: patient.city || '',
              cp: patient.postal_code || ''
            },
            emergency_contact_json: {
              nombre: patient.emergency_contact_name || '',
              relacion: patient.emergency_contact_relation || '',
          ...
          medical_record: {
            ...
            lifestyle: {
              tabaquismo: patient.smoking_status || '',
              alcohol: patient.alcohol_consumption || '',
              actividad: patient.physical_activity || '',
              sueno: patient.sleep_habits || '',
              dieta: patient.diet_habits || ''
            },
            vitals_last: {
              ...
              hr: patient.heart_rate || undefined,
              rr: patient.respiratory_rate || undefined,
              temp: patient.temperature || undefined,
              spo2: patient.oxygen_saturation || undefined
            }
          },
          encounter: {
            notes_soap: {
              S: patient.soap_subjective || '',
              O: patient.soap_objective || '',
              A: patient.soap_assessment || '',
              P: patient.soap_plan || ''
            },
            dx_codes: patient.diagnoses_cie10 || []
          },
          consent: true // Ya tiene consentimiento previo
        })
      } catch (error) {
        console.error('Error loading patient:', error)
        setErrors({ general: 'Error al cargar los datos del paciente' })
      } finally {
  ...
  const validateForm = (): boolean => {
    ...
    // Validación de edad
    if (formData.patient.date_of_birth) {
      const age = new Date().getFullYear() - new Date(formData.patient.date_of_birth).getFullYear()
      if (age < 0 || age > 120) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Éxito - redirigir a la vista del paciente
  ...
  const sections = [
    ...
    { id: 'gynob', title: 'Gineco-obstétrica' },
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <h1 className="text-2xl font-bold text-gray-900">
                Editar Paciente: {formData.patient.first_name} {formData.patient.last_name}
              </h1>
            ...
            <div className="flex items-center space-x-3">
              ...
              <button
                onClick={handleSave}
                disabled={isLoading || Object.keys(errors).some(key => key !== 'general')}
                className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50"
              >
                {isLoading ? 'Guardando...' : 'Actualizar Paciente'}
              </button>
            </div>
          </div>
      ...
      <div className="flex">
        ...
        <div className="flex-1 px-6 py-8 max-w-4xl">
          <div className="space-y-6">
            ...
            {/* Card Gineco-obstétrica - Solo si es mujer */}
